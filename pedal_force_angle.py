...
BASE_CON = f"/Users/leo/Desktop/Projet/{ESSAI}/concentric_{PUISSANCE}W"
BASE_ECC = f"/Users/leo/Desktop/Projet/{ESSAI}/eccentric_{PUISSANCE}W"
...
def compute_cycle_stats(F_pedal, crank_angle):
    ...
    eff_indice = 100*Fz/F #indice d'efficacité

    signals = np.vstack([Fx, Fy, Fz, eff_indice])  # (4,T)
...
ax = axes[3]
ax.plot(x_deg, mean_con[3], linewidth=2, label=f"Concentrique (N={ncyc_con})")
ax.fill_between(x_deg, mean_con[3]-std_con[3], mean_con[3]+std_con[3], alpha=0.20)
ax.plot(x_deg, mean_ecc[3], linewidth=2, label=f"Excentrique (N={ncyc_ecc})")
ax.fill_between(x_deg, mean_ecc[3]-std_ecc[3], mean_ecc[3]+std_ecc[3], alpha=0.20)
ax.set_title("Indice d'efficacité : (moyenne ± SD par cycle)")
ax.set_xlabel("Angle pédalier (deg)")
ax.set_ylabel("Efficacité en %")
ax.set_xlim(0, 360)
ax.grid(True, alpha=0.3)
