...
def get_muscle_names(model: biorbd.Model):
    nb_mus = model.nbMuscles()
    names = []
    for i in range(nb_mus):
        try:
            names.append(model.muscle(i).name().to_string())
        except Exception:
            names.append(f"muscle_{i}")
    return names
...
def viewer_process(model_path: str, q_params: mp.Queue):
    ...
    try:
        ...
        rr.init("muscle_torque_explorer_v2_top_only", spawn=True, recording_id=uuid.uuid4())

        # LiveModelAnimation gère sa posture q (fenêtre Q)
        ...
        def worker():
            ...
            while True:
                ...
                # DoF name
                dof_name = dof_names[dof] if 0 <= dof < len(dof_names) else f"q{dof}"
                try:
                    rr.log("analysis/selected_dof/name", rr.TextDocument(f"**DoF sélectionné**: `{dof_name}` (index {dof})"))
                except Exception:
                ...
                for m in active_ids:
                    ...
                    contribs.append((m, tau_m, r_m, a_m))
                ...
                for rank, (m, tau_m, r_m, a_m) in enumerate(top, start=1):
                    rr.log(f"analysis/top_muscles/{rank:02d}/muscle_id", rr.Scalar(float(m)))
                    rr.log(f"analysis/top_muscles/{rank:02d}/tau", rr.Scalar(float(tau_m)))
                    rr.log(f"analysis/top_muscles/{rank:02d}/activation", rr.Scalar(float(a_m)))
                    rr.log(f"analysis/top_muscles/{rank:02d}/moment_arm", rr.Scalar(float(r_m)))

                # Tableau lisible
                if len(top) > 0:
                    lines = []
                    lines.append(f"**Top {min(TOP_N, len(top))} muscles** — DoF `{dof_name}` (index {dof})  ")
                    lines.append(f"Couple total (tous muscles actifs): **{tau_total:.3f} Nm**  ")
                    lines.append("")
                    lines.append("| Rang | Muscle | a | r | τ (Nm) | |τ| |")
                    lines.append("|---:|---|---:|---:|---:|---:|")
                    for rank, (m, tau_m, r_m, a_m) in enumerate(top, start=1):
                        name = muscle_names[m] if 0 <= m < len(muscle_names) else f"muscle_{m}"
                        lines.append(f"| {rank} | {name} | {a_m:.2f} | {r_m:.4f} | {tau_m:.3f} | {abs(tau_m):.3f} |")

                    try:
                        rr.log("analysis/top_muscles/summary", rr.TextDocument("\n".join(lines)))
                    except Exception:
                        rr.log("analysis/top_muscles/summary", rr.TextLog("\n".join(lines)))
...
def ui_main(model_path: str):
    ...
    w = QtWidgets.QWidget()
    w.setWindowTitle("Muscles torque explorer V2 (Top muscles only)")
    ...
    def collect_params() -> Params:
        ...
        acts = []
        for i in range(nb_mus):
            acts.append(mus_sliders[i].value() / 100.0 if mus_checks[i].isChecked() else 0.0)
